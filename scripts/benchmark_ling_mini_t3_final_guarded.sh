...
STAMP="$(date +%F-%H%M%S)"
OUT="${ROOT}/logs/retest-2026-08-24/ling-mini-iq4-xs-t3-final-${STAMP}"
...
printf 'utc,run_id,exit_code,thermal_abort,kernel_error\n' > "${OUT}/status.csv"
{
  ...
  echo "selection=three_threads_was_the_fastest_completed_point_before_the_four_thread_sweep_hit_the_85C_guard"
  echo "workload=pp128_tg96_threads3_batch512_ubatch512_repetitions5"
  echo "gpu_layers=0"
...
while kill -0 "${job_pid}" 2>/dev/null; do
  ...
  if (( current >= THERMAL_LIMIT_MC )); then
    echo "thermal_abort,run=standardized-t3,temp_mc=${current}"
    killed=1
...
printf '%s,%s,%s,%s,%s\n' \
  "$(date -u +%FT%TZ)" standardized-t3 "${rc}" "${killed}" "${serious}" \
  >> "${OUT}/status.csv"

if (( killed != 0 || rc != 0 || serious != 0 )); then
  echo "run_failed,run=standardized-t3,rc=${rc},thermal_abort=${killed},kernel_error=${serious}"
  exit 1
fi
